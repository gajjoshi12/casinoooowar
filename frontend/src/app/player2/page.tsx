...
export default function Player2Page() {
  const playerId = '2' // Fixed player ID for this page
  ...
  const [sessionStats, setSessionStats] = useState<
    ...
    >
  >({})
  const [showResult, setShowResult] = useState(false) // State for result modal visibility
  ...
  const connectWebSocket = () => {
    try {
      wsRef.current = new WebSocket('ws://192.168.181.109:6789')
  ...
  // Watch for result changes to trigger modal with delay
  useEffect(() => {
    if (playerData?.result) {
      // Clear any existing timeout
      if (resultTimeoutRef.current) clearTimeout(resultTimeoutRef.current)

      // Delay showing the result to allow for card animations (dealer card is 0.6s)
      resultTimeoutRef.current = setTimeout(() => {
        setShowResult(true)
      }, 1500) // 1.5s delay to be safe and impactful
    } else {
      setShowResult(false)
      if (resultTimeoutRef.current) clearTimeout(resultTimeoutRef.current)
    }
            ...
            <div className='bg-[#911606] border-4 border-[#d4af37] p-4 sm:p-6 md:p-8 w-full grow flex flex-col rounded-lg shadow-xl'>
              ...
              <div className='text-center mb-6 sm:mb-8'>
                ...
                {playerData && (
                  <div className='flex justify-center'>
                    <div
                      className={`inline-block px-4 sm:px-6 py-1.5 sm:py-2 ${playerData.status === 'active'
                        ? 'bg-[#7a1105]'
              ...
              <div className='bg-[#a42210] border-2 border-[#d4af37] p-4 sm:p-6 rounded-xl mb-6 sm:mb-8 shadow-md'>
                ...
                <div className='flex justify-center items-center gap-4 min-h-[120px]'>
                  ...
                  <div className='flex flex-col items-center relative w-32 h-44'>
                    {gameState.dealer_card && (
                      <AnimatedCard card={gameState.dealer_card} size='large' delay={0.6} />
                    )}
                  </div>
                  ...
                  {hasWarData && (
                    <div className='flex flex-col items-center relative w-32 h-44'>
                      {gameState.war_round?.dealer_card && (
                        <AnimatedCard
                          card={gameState.war_round.dealer_card}
                          size='large'
                          index={1}
                        />
                      )}
              ...
              <div className='text-center'>
                <div className='bg-[#a42210] p-4 sm:p-6 rounded-xl mb-6 sm:mb-8 shadow-md'>
                  ...
                  {playerData ? (
                    <div className='flex justify-center items-center gap-4'>
                      ...
                      <div className='flex flex-col items-center relative w-32 h-44'>
                        {playerData.card && (
                          <AnimatedCard card={playerData.card} size='large' />
                        )}
                      </div>
                      ...
                      {(playerData.war_card || hasWarData) && (
                        <div className='flex flex-col items-center relative w-32 h-44'>
                          {playerData.war_card ? (
                            <AnimatedCard
                              card={playerData.war_card}
                              size='large'
                              index={1}
                            />
                          ) : gameState.war_round?.players[playerId] ? ( // Show opponent's war card if available
                            <AnimatedCard
                              card={gameState.war_round.players[playerId]!}
                              size='large'
                              index={1}
                            />
                          ) : null}
                ...
                {playerData && (
                  <div className='space-y-4'>
                    {/* Result */}
                    {/* Result Modal is now handled outside, but we keep the structure clean */}
                    <ResultModal
